...
usersInScore = score.map(function (a) {
    return a.name;
});
usernames = users.map(function (a) {
    return a.username;
});
            ...
            if (score[i].name == showUser) {
                for (var k = 0; k < users.length; k++) {
                    if (users[k].username == showUser) {
                        gamesInScore = score[i].games.map(function (a) {
                            return a.game;
                        });
...
function next() {
    ...
    trueWords = trueAnswersArray.map(function (a) {
        return a.word;
    });
    ...
    if (words.length > trueAnswersArray.length) {
        ...
        for (var j = 0; j < score.length; j++) {
            gamesInScore = score[j].games.map(function (a) {
                return a.game;
            });
            timeInScore = score[j].games.map(function (a) {
                return a.time;
            });
...
function check() {
    ...
    if (selectedWord === answerLetter.join("")) {
        ...
        for (var j = 0; j < score.length; j++) {
            gamessInScore = score[j].games.map(function (a) {
                return a.game;
            });
            timeInScore = score[j].games.map(function (a) {
                return a.time;
